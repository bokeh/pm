...
*Attending: Bryan, Ian, Mateusz, Philipp, Timo*

### Agenda

- Next weekly meeting on Jan 5

### Updates

#### Mateusz

- finalizing support for shadow DOM (PR #11239)
- continued work on bokehjs' "devtools" (PR #11871)
- working on layout related tasks
- investigated webgl support issues in Chromium 96+
- fixed rendering of data URL image glyphs (PR #11888)

#### Ian

- PR for regression test for webgl (#11884)

#### Timo

- CZI-related issue, PR cleanup
- Submitting final report to NF
- started SDG tutorial discussion

#### Bryan

- PR for reduce import times (#11891)
- created `bokeh/tutorials` repo
- created discussion for CI/testing suggestion (#11893)
...
*Attending: Bryan, Greg, Ian, Iury, Mateusz, Philipp, Timo*
...
- PR to pin Django 3.x
- PR review / comment
- `Block` PR ready for review

#### Iury

- Working on refactor for Mathjax
  - issues with image loading async
- PR to parse SVG path output from Mathjax

#### Timo

- Final report for CZI
- Docs PR updates

#### Philipp

- looking at session close events

#### Ian

- Adding more LaTeX examples
- Docker container PR

#### Mateusz

- implemented bokehjs' "devtools" (PR #11871)
- working on layout related tasks
- porting new latex examples to bokehjs

#### Greg

- PR for whitespace control in templates (WIP)
- New contributor / GFI interaction
...
- Python / NumPy versions for 3.0
  - proposal: 3.8 and 1.18.0

- Webdriver install woes
  - https://discourse.bokeh.org/t/how-to-install-setup-gecko-webdriver-on-fedora-linux-pycharm/8688
  - https://pypi.org/project/webdriver-manager/

- Holiday meeting schedule
  - proposal: skip Dec 29

### Updates

#### Bryan

- Opened discussion PR for `Block` glyph #11844
- Finished simplified CI config with composite actions #11833
- Created Onebox plugin for Bokeh docs site
  - https://github.com/bokeh/discourse-bokeh-onebox
- Looking to start `src` layout soon

#### Mateusz

- improvements to module bundling (PR #11844)
  - experiments with bundling webworkers and wasm
- replaced `setter_id` mechanism in bokehjs (PR #11836)
  - implemented rudimentary bokehjs server for testing
- continued work on shadow DOM (PR #11239)
- started working on model/property explorer
  - primarily for boosting layout/CSS work
  - will be useful on its own at some point
- did a bit more of profiling around slow tooltips

#### Ian

- Another WebGL PR
- Docker work, conveniences and configuration
  - one remaining blocker
- Thinking about new Mathtext examples for next week

#### Iury

- Text fallback PR
- PR to Refactor mathtext w/o CDN mathjax
  - for canvas models and DOM models

####

- Feedback about CZI RFP for single-cell bioscience
  - not much opportnity to drive improvements directly to Bokeh

#### Timo

- Finishing up CZI final report
- Final docs work for CZI
...
*Attending: Bryan, Iury, Mateusz, Philipp, Timo*

### Agenda

- CZI report in the next few weeks

### Updates

#### Bryan

- 2.4.2 released, docs updates forwarded
- Added Python 3.10 to CI
- PR open to simplfiy GH CI config

#### Mateusz

- improved initialization-time change detection (PR #11828)
  - explored other changes (e.g. replacing `_invalidate_all_models()`)
- working on finalizing shadow DOM support (PR #11239)
- working on bundling improvements
  - proper dependencies between bundles
  - will allow multi-bundle support in extensions
  - splitting up bokeh (especially core from vis)
  - reinvestigating ESM support in bundles
- working on a proper grid plot API
- gave headless Firefox another chance
- fixed branches in CI workflows (PR #11821)
  - bokehjs' tests didn't run on `push` for months
- forced webgl in bokehjs' tests (PR #11823)
  - tests will fail if webgl is requested but not available
  - can be configured with `Bokeh.settings`
- worked around issues with Chrome 96 on Linux (PR #11823)
  - Chrome 95 is installed from `bokeh/chromium` repository
- migrated bokehjs to using npm workspaces (PR #11816)
  - split up `bokehjs/package.json`
  - moved web "runtime" dependencies to `devDependencies`

#### Timo

- Waiting on feedback for BokehJS docs #11709 (reply: all good)

#### Philipp

- Panel 0.12.5 released

#### Iury

- Working end of the week on Bokeh
  - Mateusz will review

#### Ian

- Working on WebGL improvments
- Experiments with docker image

### Actions

## 17-Nov

*Attending: Bryan, Ian, Jim, Mateusz, Philipp, Timo*

### Agenda

- Next release
  - 2.4.2 dev build as soon as backports completed
  - release after OK from Philipp

### Updates

#### Mateusz

- fixing regressions for 2.4.2 (PR #11469, #11807, #11808)
- improved handling of data ranges (PR #11802)
- removed lagacy code paths from build/compiler (PR #11812)
- clarified internals of message handling in bokehjs (PR #11813)
- continued work on dynamic loading of bundles
  - experimented with further splitting up bokehjs

#### Bryan

- PR for adding favicon
- Two rounds of backport PRs (one still open)
- Cache busting for custom.css

#### Philipp

- Diagnosing regressions, testing

#### Timo

- Updating GH issues to use new forms
- Add OS info to `bokeh info`
- Created *Good First Issue* template (in Wiki*)

#### Ian

- PR for webgl dashed lines
- More webGL PRs in the next few weeks
- Experimenting with docker containter
...
- Organizing PyData Global Sprint
...
- bokeh 2.4 compatibility in holoviz
