...
- preliminary PR for key bindings management (PR #14172): very early WIP, focus so far on design, should ultimately allow us to controll plots exclusively with the keyboard (sequential key handling similar to vim). This will also include a status bar for feedback.
...
- Regression fixing (tile rendering / annotations). This is related to a whole class of problems
- added support for custom marker types to `Scatter` glyph (PR #14)
